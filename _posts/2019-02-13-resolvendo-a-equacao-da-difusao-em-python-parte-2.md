...
No [último post](https://tarcisiofischer.github.io/2019-02-10/resolvendo-a-equacao-da-difusao-em-python-parte-1),
...
Para relembrar o que era a matriz $A$, mostro novamente as dependências de um
...
- c \Delta t f^{2, 1}
=
\Delta s^2 f_o^{2, 1} + \Delta s^2 \Delta t s^{2, 1}
$$
...
Perceba o calculo dos índices da matriz. Como cada linha da matriz A se refere
à dependência entre o elemento (i, j) = A[i * N + j, i * N + j] e todos os outros elementos
(x, y) do nosso problema, essa conversão é necessária. A imagem abaixo ilustra
essa conversão de índices. Na imagem, a linha 1 da matriz A (à direita) se
...
na matriz A. Isso é análogo ao que já fizemos para a diagonal principal, porém,
somando ou decrementando os índices para cada caso. Obtemos, então, o seguinte
código:
...
O que acontece é que estamos tentando acessar um índice fora do vetor A. Sendo
...
# Utilizando funções lambda para a conversão de índices

Uma possibilidade que eu, particularmente, acho interessante, é utilizar uma
função auxiliar para fazer a transformação de índices. Assim, o código fica, na
minha visão, um pouco mais legível. Em Python, é possível definir uma função
anônima
[lambda](https://docs.python.org/3/tutorial/controlflow.html#lambda-expressions).
Uma função lambda deve ser atribuída a uma variável, para que possa ser
...
ao índice 9, que estava fora do tamanho do array A (que tinha tamanho
$9 \times 9$). Porém, a linha imediatamente anterior não apresentava erro
explícito, ou seja, o código abaixo é válido sempre (Sugiro tentar executar e
...
por que eventualmente, os calculos vão resultar em índices negativos. Quando
...
Esse post já está demasiado longo e, portanto, não vou conseguir entrar a
fundo nessa discussão. Se alguém tiver interesse sobre isso, por favor, me
envie uma mensagem em alguma rede social, que eu preparo um
...
aplicações nunca chegamos a ter problemas. Considere, porém, o
...
grande maioria dos elementos são zeros, ou seja, a matriz é
[esparsa](https://en.wikipedia.org/wiki/Sparse_matrix) (Ou, nesse
caso específico, ela é uma matriz
[esparsa banda](https://en.wikipedia.org/wiki/Sparse_matrix#Banded)). A
quantidade de elementos não-zeros nessa matriz é aproximadamente a soma da
diagonal principal com 4 diagonais secundárias, ou seja, sendo relaxado no
cálculo, posso dizer que é algo em torno de $5 \cdot N^2$, o que, pra $N=100$,
resulta em um valor em torno de 400Kb.

Podemos escolher uma estrutura de dados para tirar proveito dessa
propriedade, nesse caso, as estruturas disponíveis na
[scipy.sparse](https://docs.scipy.org/doc/scipy/reference/sparse.html) são boas
sugestões. Existem várias formas e fazer isso, por exemplo, utilizando
diretamente a estrutura de dados crua
[coo_matrix](https://docs.scipy.org/doc/scipy/reference/generated/scipy.sparse.coo_matrix.html),
ou tentando utilizar a função auxiliar
...
em Python e gerar a matriz $A$ totalmente utilizando slicing. No
